  ...
  uint8_t i=0;
	...
	switch(val){
		...
		case 2:
			...
			if(Mo[mLine].Pow==RUN){
				Mo[mLine].Pow=SLEEP;
				EE_WriteVariable(mLine*4+3,SLEEP);
			}
			Mo[mLine].MCH=mMCH;
			Mo[mLine].Speed=MCH_Set[mMCH].Speed;
			Mo[mLine].Di=MCH_Set[mMCH].Di;
			EE_WriteVariable(mLine*4,Mo[mLine].MCH);
			EE_WriteVariable(mLine*4+1,Mo[mLine].Speed);
			EE_WriteVariable(mLine*4+2,Mo[mLine].Di);
			if(mLine>=MOTOR_NUM){
		...
		case 3:
			...
			if(mLine>MOTOR_NUM)
				...
				sprintf(str,"tpd99%d%d%d%d",mMCH,mTPD,mDir,SLEEP);
			USART_SendString(USART2,str);
		...
		case 5:
			if(Mo[mLine].Pow==RUN){
				Mo[mLine].Pow=SLEEP;
				srf_pow[mLine]=Mo[mLine].Pow;
				EE_WriteVariable(mLine*4+3,SLEEP);
			}
			else
			{
				Mo[mLine].Pow=RUN;
				EE_WriteVariable(mLine*4+3,RUN);
				srf_pow[mLine]=Mo[mLine].Pow;
				if(mLine<6)
				motor_res(mLine);
			}
...
//		TIM2->CCR1=ccr1;
//		TIM2->CCR2=ccr2;	
	TIM2_OCInitStructure.TIM_Pulse = ccr1;	
	TIM_OC1Init(TIM2, &TIM2_OCInitStructure);
	TIM2_OCInitStructure.TIM_Pulse = ccr2;	
...
uint32_t GetDistance(void){
	...
	//USART_SendString(USART1,"range_ok=0\n");
	while(!range_ok){
		if(tim_10us>3000)
			range_ok=1;
		delay_ms(1);
	}
	//USART_SendString(USART1,"range_ok=1\n");
	val=pulse_num*5;
	distance=val/29;
